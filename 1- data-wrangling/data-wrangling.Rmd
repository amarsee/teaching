...
* `select()` - Keeping/dropping variables
...
ach_profile %>%
    filter(system_name != "State of Tennessee") %>%
    summarise(
        mean_grad = mean(Graduation, na.rm = TRUE),
...
ach_profile %>%
    filter(system_name != "State of Tennessee") %>%
    mutate(
        median_pct_ED = median(Pct_ED, na.rm = TRUE),
...
ach_profile %>%
    rowwise() %>%
    mutate(
        Math_avg = mean(c(Math, AlgI, AlgII), na.rm = TRUE),
...
Key variable names don't have to match between the two data frames:
...
Create a data frame with the number of districts at each TVAAS Composite level, by CORE region.
