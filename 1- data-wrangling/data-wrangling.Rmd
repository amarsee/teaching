...
The first argument of all `dplyr` and `tidyr` functions are the data frame you want to manipulate, so piping makes for more readable code:
...
Sort alphabetically by CORE region, then by Algebra I proficiency in descending order. Then, keep just the district name, Algebra I proficiency, and CORE region variables.
...
What is the `ach_profile` data frame wide by? Long by?
...
`tidyr`'s functions are based on the idea of **key-value pairs**.

A **key** describes what information your data contains, and the **value** is the information itself.

## Some terminology

Values of key variables might be spread across columns or rows:
...
``` {r, eval = FALSE}
gather(ach_profile, key = outcome, value = value, AlgI:Dropout)
```

> * The first argument to `gather()` is a data frame.
> * The second is the name of a new variable which will take the values of the key.
> * The third is the name of a new variable which will take the values in key-value pairs.
> * The fourth is a set of variable names which are the values of your key.
