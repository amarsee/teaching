...
``` {r, echo = FALSE}
districts %>%
    gather(outcome, value, alg_1:dropout) %>%
...
``` {r, eval = FALSE}
gather(districts, outcome, value, alg_1:dropout)
```
...
> * The fourth is the columns you want to convert to a column.
...
``` {r, eval = FALSE}
spread(districts_long, system_name, value)
```
