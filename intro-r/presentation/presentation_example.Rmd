...
leaflet() %>%
    addTiles() %>%
    setView(-86.767239, 36.120877, zoom = 10) %>%
...
ach_plot <- ach_profile %>%
    ...
    gvisColumnChart(xvar = "Subject", yvar = c("Davidson County", "State"), options = list(title = "Achievement in 3-8 Subjects", width = 1000, height = 550))
...
eoc_plot <- ach_profile %>%
    ...
    spread("District", "Percent P/A") %>%
    gvisColumnChart(xvar = "Subject", yvar = c("Davidson County", "State"), options = list(title = "Achievement in EOC Subjects", width = 1000, height = 550))
