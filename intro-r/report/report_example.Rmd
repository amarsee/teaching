...
ach_profile <- read_csv("achievement_profile_data_with_CORE.csv") %>%
    ...
    filter(District %in% c("State of Tennessee", "Davidson County")) %>%
    mutate(District = ifelse(District == "State of Tennessee", "State", District))

subgroup_data <- read_csv("subgroup_achievement.csv") %>%
    rename(District = system_name) %>%
    filter(District %in% c("State of Tennessee", "Davidson County")) %>%
    mutate(District = ifelse(District == "State of Tennessee", "State", District))
...
eoc_plot <- ach_profile %>%
    ...
    spread(District, `Percent P/A`) %>%
    gvisColumnChart(xvar = "Subject", yvar = c("Davidson County", "State"), options = list(title = "Achievement in EOC Subjects", width = 900, height = 500))
...
Here is an interactive graph. Use the dropdown to select a subgroup.
...
shinyApp(
    ...
    ui <- fluidPage(
        selectInput("subgroup", label = "Select a subgroup:", choices = unique(subgroup_data$subgroup)),
        htmlOutput("chart38")
    ),

    server <- function(input, output){

    output$chart38 <- renderGvis({
        subgroup_data %>%
            filter(subgroup == input$subgroup) %>%
            select(District, Math, ELA, Science) %>%
            gather(Subject, `Percent P/A`, 2:4) %>%
            spread(District, `Percent P/A`) %>%
            gvisColumnChart(xvar = "Subject", yvar = c("Davidson County", "State"), options = list(title = "Achievement in 3-8 Subjects", width = 750, height = 500, vAxis = "{baseline: 0, maxValue: 100}"))
        })
    },
