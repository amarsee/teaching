...
ach_profile <- read_csv("achievement_profile_data.csv") %>%
    filter(system_name == "State of Tennessee" | system_name == "Davidson County")

ach_profile[ach_profile$system_name == "State of Tennessee", ]$system_name <- "State"
...
Davidson County is a school district serving `r ach_profile[ach_profile$system_name == "Davidson County", ]$Enrollment` students. Its expenditures in 2015 were `r paste0("$", sprintf("%.2f", ach_profile[ach_profile$system_name == "Davidson County", ]$Per_Pupil_Expenditures))` per student.
...
ach_profile %>%
    select(one_of(c("system_name", "AlgI", "AlgII", "BioI", "Chemistry", "EngI", "EngII", "EngIII"))) %>%
    rename("District" = system_name, "Algebra I" = AlgI, "Algebra II" = AlgII, "Biology I" = BioI, "English I" = EngI, "English II" = EngII, "English III" = EngIII) %>%
