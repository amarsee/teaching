...
```{r}
nnet_6 <- read_rds("models/nnet_6.rds")
nnet_7 <- read_rds("models/nnet_7.rds")
nnet_8 <- read_rds("models/nnet_8.rds")
...
Split test set into (preprocessed) predictors and outcomes:

``` {r}
test_x_6 <- preprocess_test(test_6)
test_x_7 <- preprocess_test(test_7)
test_x_8 <- preprocess_test(test_8)

test_y_6 <- test_6$ready_grad
test_y_7 <- test_7$ready_grad
test_y_8 <- test_8$ready_grad
```
...
```{r, eval = FALSE}
map2(
    .x = list(nnet_6, nnet_7, nnet_8),
    .y = list(test_x_6, test_x_7, test_x_8),
    .f = predict
)
```
...
map2(
    .x = .,
    .y = list(test_y_6, test_y_7, test_y_8),
    .f = ~ .x == .y) %>%
map_dbl(mean)
```
...
pmap(
    .l = list(
        models = list(nnet_6, nnet_7, nnet_8),
        test_sets = list(test_6, test_7, test_8)
    ),
    .f = predict
)
```
...
This means we can use `map_*` to apply functions over columns of a data frame.
...
You can use `map`s on list-columns in a `tibble`:

```{r}
model_df <- tibble(
    grade = c(6, 7, 8),
    test_set = list(test_x_6, test_x_7, test_x_8),
    model = list(nnet_6, nnet_7, nnet_8)
)
```
...
```{r, eval = FALSE}
preprocess_test(test_6)
preprocess_test(test_7)
preprocess_test(test_8)
```
...
```{r, eval = FALSE}
map(.x = list(test_6, test_7, test_8), .f = preprocess_test)
```
...
```{r, eval = FALSE}
map(.x = list(test_6, test_7, test_8), .f = "ready_grad")
```
...
fit_nnet <- function(grade) {
    ...
    x <- predict(train_preprocess, x)

    train(x = x, y = y, method = "nnet", metric = "ROC", trControl = train_controls)
}
```
...
walk(
    .x = list(nnet_6, nnet_7, nnet_8),
    .f = write_rds("models/nnet_6.rds", "models/nnet_7.rds", "models/nnet_8.rds")
)
```
